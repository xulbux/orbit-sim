...
export class Engine {
  public bodies: Body[] = [];
  public gravitationalConstant = 100;
  public timeScale = 1;
  ...
  public calculateGravity(body1: Body, body2: Body): Vector2 {
    ...
    if (body1.type === BodyType.BLACK_HOLE || body2.type === BodyType.BLACK_HOLE) {
      // Cap the minimum radius to the black hole's radius to prevent extreme sling-shotting at the singularity
  ...
  public static applyBlackHoleSpiral(body: Body, blackHole: Body, deltaTime: number): void {
    ...
    // If close enough to black hole, simulate accretion disk drag (friction)
    if (distance < effectRadius) {
      // Calculate how close the planet is (0 at edge, 1 at center)
      const proximity = 1 - distance / effectRadius;

      // Apply accretion disk drag so gravity can pull it in naturally in a spiral
      ...
      if (distance < blackHole.radius) {
        ...
        // to allow the shrinking animation to play over roughly ~500ms
